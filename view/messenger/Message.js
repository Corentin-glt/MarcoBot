...
class Message {
  ...
  sendSingle(message) {
    console.log(message);
    if (typeof message === 'object' && typeof message.messagePause ===
      'number') {
      return new Promise(
        resolve => setTimeout(resolve, parseInt(message.messagePause, 10)));
    }
    const messageBody = {
      recipient: {
        id: this.senderID
    ...
    if (typeof this.message === 'string') {
      ...
      this.message.forEach(msg => {
        if (typeof msg === 'string') {
          this.messages = this.messages.concat(this.breakTextAndReturnFormatted(msg));
        } else {
  ...
  sendAll() {
    if (!this.messages.length || this.messages.length === 0) {
      ...
      return this.sendSingle(this.messages.shift()).then(res => {
