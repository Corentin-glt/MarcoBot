...
class Init {
  ...
  handleEntry() {
    this.entry.forEach(item => {
      item.messaging.forEach(event => {
        ...
        apiGraphql
          .sendQuery(accountQuery.queryPSID(mainEvent.senderId))
          .then(res => {
  ...
  handleGet(event, res) {
    const account = new Account(
      res.accountMessenger.PSID,
      res.accountMessenger.locale.split("_")[0],
      res.accountMessenger.timezone
    );
    ...
    apiGraphql
      .sendQuery(userQuery.queryUserByAccountMessenger(account.PSID))
      .then(res => {
